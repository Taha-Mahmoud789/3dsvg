 ...
 * =============================================================================
 *
 * Dialog for exporting the current 3D scene as a mesh file. Surfaces GLB
 * and STL as the recommended formats; OBJ and PLY live under an "Other
 * formats" disclosure for niche workflows.
 */
...
import type { Export3DFormat } from "@/components/svg-to-3d-canvas";
...
export function DownloadDialog({ open, onOpenChange, onDownload }: DownloadDialogProps) {
  const [format, setFormat] = useState<Export3DFormat>("glb");
  const [otherOpen, setOtherOpen] = useState(false);

  const activeFormat =
    [...PRIMARY_FORMATS, ...SECONDARY_FORMATS].find((f) => f.value === format) ??
    PRIMARY_FORMATS[0];
        ...
        <div className="space-y-2 py-2">
          ...
          <Collapsible open={otherOpen} onOpenChange={setOtherOpen}>
            <CollapsibleTrigger className="flex w-full items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors pt-1">
              <ChevronDown className={`h-3.5 w-3.5 transition-transform ${otherOpen ? "rotate-0" : "-rotate-90"}`} />
              Other formats
            </CollapsibleTrigger>
